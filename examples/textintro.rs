...
impl TextIntroDemo {
	fn new(g: &mut shade::Graphics) -> TextIntroDemo {
		...
		TextIntroDemo { font, storyboard, epoch }
	}

	fn draw(&mut self, g: &mut shade::Graphics, viewport: Bounds2i) {
		let elapsed = self.epoch.elapsed().as_secs_f32() % LOOP_DURATION;
		...
		for item in &self.storyboard {
			...
			self.add_text_plane(g, &mut buf, item.text, properties.scale, properties.plane_transform(), &scribe);
		}
	...
	fn storyboard() -> Vec<AnimatedText> {
		...
		storyboard.push(AnimatedText {
			...
			animations: vec![
				Animation::AlphaWindow { start: 0.0, end: 4.8, fade: 1.0 },
			],
		});
		...
		storyboard.push(AnimatedText {
			...
			animations: vec![
				Animation::AlphaWindow { start: 22.0, end: LOOP_DURATION, fade: 0.8 },
			],
		});
...
impl App {
	fn new(event_loop: &winit::event_loop::ActiveEventLoop, size: winit::dpi::PhysicalSize<u32>) -> Box<App> {
		...
		let demo = TextIntroDemo::new(opengl.as_graphics());
			...
			Event::WindowEvent { event, .. } => match event {
				...
				WindowEvent::RedrawRequested => {
					if let Some(app) = app.as_deref_mut() {
						...
						app.window
							.surface
							.swap_buffers(&app.window.context)
							.unwrap();
					}
				}
				_ => {}
