...
impl TestShaderInterface {
	fn new(files: &[(&'static str, &'static str)]) -> TestShaderInterface {
		let files = files.iter().copied().collect();
		TestShaderInterface { files }
	}
}

impl IShaderInterface for TestShaderInterface {
	fn include_source(&mut self, name: &str) -> io::Result<String> {
		self.files.get(name).copied().map(String::from).ok_or_else(|| io::Error::from(io::ErrorKind::NotFound))
	}
}
...
fn compile_shader_source_injects_stage_and_backend_defines() {
	...
	let output = lang::compile("main.glsl", 0, ShaderKind::Vertex, "330 core", "GLSL_CORE", &[
...
#[test]
fn compile_shader_source_ignores_leading_blank_lines_before_version() {
	let mut interface = TestShaderInterface::new(&[("main.glsl", "\n\n#version unified 330 core, 300 es\nvoid main() {}\n")]);
	let output = lang::compile("main.glsl", 0, ShaderKind::Vertex, "300 es", "GLSL_ES", &[], &mut interface);

	assert!(output.starts_with("#version 300 es\n"));
	assert!(output.contains("void main() {}"));
	assert!(output.contains("#line 4 0\nvoid main() {}"));
}
