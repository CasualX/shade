...
fn gl_attributes(dynamic_vao: GLuint, shader: &GlShader, data: &[crate::DrawVertexBuffer], map: &ResourceMap<GlVertexBuffer>) {
...
pub fn arrays(this: &mut GlGraphics, args: &crate::DrawArgs) -> Result<(), crate::GfxError> {
	...
	gl_check!(gl::UseProgram(shader.program));
	gl_attributes(this.dynamic_vao, shader, args.vertices, &this.vbuffers);
