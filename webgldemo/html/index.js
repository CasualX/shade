const MODULE_NAME = "shadegldemo.wasm";
		...
		try {
			...
			const imports = {
				webgl: {
					...
					uniform1fv(location, count, value) { gl.uniform1fv(handles.get(location), new Float32Array(memory.buffer, value, count)); },
					uniform2fv(location, count, value) { gl.uniform2fv(handles.get(location), new Float32Array(memory.buffer, value, count)); },
					uniform3fv(location, count, value) { gl.uniform3fv(handles.get(location), new Float32Array(memory.buffer, value, count)); },
					uniform4fv(location, count, value) { gl.uniform4fv(handles.get(location), new Float32Array(memory.buffer, value, count)); },
					uniform1iv(location, count, value) { gl.uniform1iv(handles.get(location), new Int32Array(memory.buffer, value, count)); },
					uniform2iv(location, count, value) { gl.uniform2iv(handles.get(location), new Int32Array(memory.buffer, value, count)); },
					uniform3iv(location, count, value) { gl.uniform3iv(handles.get(location), new Int32Array(memory.buffer, value, count)); },
					uniform4iv(location, count, value) { gl.uniform4iv(handles.get(location), new Int32Array(memory.buffer, value, count)); },
					uniformMatrix2fv(location, count, transpose, value) {
						gl.uniformMatrix2fv(handles.get(location), transpose, new Float32Array(memory.buffer, value, count));
					},
					uniformMatrix2x3fv(location, count, transpose, value) {
						gl.uniformMatrix2x3fv(handles.get(location), transpose, new Float32Array(memory.buffer, value, count));
					},
					uniformMatrix2x4fv(location, count, transpose, value) {
						gl.uniformMatrix2x4fv(handles.get(location), transpose, new Float32Array(memory.buffer, value, count));
					},
					uniformMatrix3x2fv(location, count, transpose, value) {
						gl.uniformMatrix3x2fv(handles.get(location), transpose, new Float32Array(memory.buffer, value, count));
					},
					uniformMatrix3fv(location, count, transpose, value) {
						gl.uniformMatrix3fv(handles.get(location), transpose, new Float32Array(memory.buffer, value, count));
					},
					uniformMatrix3x4fv(location, count, transpose, value) {
						gl.uniformMatrix3x4fv(handles.get(location), transpose, new Float32Array(memory.buffer, value, count));
					},
					uniformMatrix4x2fv(location, count, transpose, value) {
						gl.uniformMatrix4x2fv(handles.get(location), transpose, new Float32Array(memory.buffer, value, count));
					},
					uniformMatrix4x3fv(location, count, transpose, value) {
						gl.uniformMatrix4x3fv(handles.get(location), transpose, new Float32Array(memory.buffer, value, count));
					},
					uniformMatrix4fv(location, count, transpose, value) {
						gl.uniformMatrix4fv(handles.get(location), transpose, new Float32Array(memory.buffer, value, count));
					},
				...
				env: {
					// Placeholder for manual imports (e.g. memory, functions)
					consoleLog(msg_ptr, msg_len) {
						const msg = new TextDecoder().decode(new Uint8Array(memory.buffer, msg_ptr, msg_len));
						console.log(msg);
					},
			...
			alert(`Failed to load the WebAssembly module. Please check that '${MODULE_NAME}' is present.`);
		}
	}
